document.addEventListener('DOMContentLoaded', function() {
	// Mostrar la capa de carga al principio
    const overlay = document.getElementById('overlay');
    overlay.classList.remove('hide');

    // Ocultar la capa de carga después de 5 segundos
    setTimeout(function () {
...
function initializeCells() {
	activateCellType('.bg-color-7', 'bgColor7Enabled', ['.bg-color-0', '.bg-color-1', '.bg-color-2', '.bg-color-3', '.bg-color-4',, '.bg-color-5',, '.bg-color-6']);
	activateCellType('.bg-color-6', 'bgColor6Enabled', ['.bg-color-0', '.bg-color-1', '.bg-color-2', '.bg-color-3', '.bg-color-4']);
